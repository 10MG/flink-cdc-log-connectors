...
package cn.tenmg.cdc.log.debezium.table;
...
import cn.tenmg.cdc.log.debezium.utils.TemporalConversions;
...
public final class RowDataDebeziumDeserializeSchema
    ...
    public void deserialize(SourceRecord record, Collector<RowData> out) throws Exception {
        ...
        if (op == Envelope.Operation.CREATE || op == Envelope.Operation.READ) {
            ...
            insert.setField(insert.getArity() - 1, StringData.fromString(op.code()));
            emit(record, insert, out);
        } else if (op == Envelope.Operation.DELETE) {
            GenericRowData delete = extractBeforeRow(value, valueSchema);
            validator.validate(delete, RowKind.INSERT);
            delete.setRowKind(RowKind.INSERT);
            delete.setField(delete.getArity() - 1, StringData.fromString(op.code()));
            emit(record, delete, out);
        } else {
            GenericRowData before = extractBeforeRow(value, valueSchema);
            validator.validate(before, RowKind.UPDATE_BEFORE);
            before.setRowKind(RowKind.UPDATE_BEFORE);
            before.setField(before.getArity() - 1, StringData.fromString(op.code()));
            emit(record, before, out);

            GenericRowData after = extractAfterRow(value, valueSchema);
            validator.validate(after, RowKind.UPDATE_AFTER);
            after.setRowKind(RowKind.UPDATE_AFTER);
            after.setField(after.getArity() - 1, StringData.fromString(op.code()));
